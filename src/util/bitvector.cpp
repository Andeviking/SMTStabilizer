...
bool BitVector::is_ones() const {
    ...
    if (is_gmp()) {
        ...
        m = static_cast<uint64_t>(mp_bits_per_limb) - d_size % static_cast<uint64_t>(mp_bits_per_limb);
...
bool BitVector::is_min_signed() const {
    ...
    if (is_gmp()) {
        ...
        if (d_val_uint64 != ((uint64_t)1 << (d_size == 64 ? 63 : ((d_size % 64) - 1)))) {
            return false;
        }
    }
    return true;
...
bool BitVector::is_sadd_overflow(const BitVector& bv) const {
    ...
    return (is_signed_bv0 && is_signed_bv1 && !is_signed_add) || (!is_signed_bv0 && !is_signed_bv1 && is_signed_add);
}
...
bool BitVector::is_ssub_overflow(const BitVector& bv) const {
    ...
    return (is_signed_bv0 && !is_signed_bv1 && !is_signed_sub) || (!is_signed_bv0 && is_signed_bv1 && is_signed_sub);
}
...
bool BitVector::is_smul_overflow(const BitVector& bv) const {
    ...
    return mul.signed_compare(BitVector::mk_min_signed(d_size).ibvsext(d_size)) < 0 || mul.signed_compare(BitVector::mk_max_signed(d_size).ibvsext(d_size)) > 0;
}
...
void BitVector::bvudivurem(const BitVector& bv,
                           BitVector* quot,
                           BitVector* rem) const {
    assert(!is_null());
    assert(!bv.is_null());
...
BitVector::get_limb(void* limb, uint64_t nbits_rem, bool zeros) const {
    ...
    if (is_gmp()) {
        ...
        if (d_val_uint64 == 0) {
            ...
            n_limbs = (mp_bits_per_limb >= bits_per_limb) || (d_val_uint64 >> mp_bits_per_limb) == 0
                          ? 1
