...
bool BitVector::fits_in_size(uint64_t size, const std::string &str,
                             uint32_t base) {
  bool is_neg = str[0] == '-';
  bool res;
...
BitVector BitVector::bvite(const BitVector &c, const BitVector &t,
                           const BitVector &e) {
  assert(!c.is_null());
  assert(!t.is_null());
  assert(!e.is_null());
...
BitVector &BitVector::ibvextract(const BitVector &bv, uint64_t idx_hi,
                                 uint64_t idx_lo) {
  assert(!bv.is_null());
...
BitVector &BitVector::ibvite(const BitVector &c, const BitVector &t,
                             const BitVector &e) {
  assert(!c.is_null());
  assert(!t.is_null());
  assert(!e.is_null());
...
void BitVector::bvudivurem(const BitVector &bv, BitVector *quot,
                           BitVector *rem) const {
  assert(!is_null());
  assert(!bv.is_null());
  assert(quot != rem);
